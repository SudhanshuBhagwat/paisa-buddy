...
import type { AccountWithBalance } from '@paisa-buddy/shared/types/account'
...
import { filterTransactions, getRecentCategories } from '@paisa-buddy/shared/logic/transaction'
import { deriveDashboardStats } from '@paisa-buddy/shared/logic/dashboard'
  ...
  return (
    <main className="w-full mobile-content-pb md:pb-0 md:pt-[66px] min-h-dvh">
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0, height: '100%', overflowY: 'auto' }}>
          ...
          <div style={{ flexShrink: 0, ...(txs.length > 0 ? CARD : {}), paddingTop: txs.length > 0 ? 10 : 0, paddingBottom: 20 }}>
            {txs.length === 0 ? (
              <div style={{ marginTop: '4rem' }}>
                <HomeEmptyState onAddManually={() => setModalOpen(true)} />
              </div>
            ) : (
              <>
                {selectedDate && (
